...
class SamsungActiveKeyAccessibilityService : AccessibilityService() {
     ...
     * the held state. If the service is interrupted while a key is
     * physically held, we may never see the matching key-up — which
     * would strand a down edge in the dispatcher (TX stuck engaged)
     * and make the next press look like a stuck "already held" duplicate.
     * Mirror the UP edge if a button was held, then clear local state.
     */
    ...
    /**
     * Fire a synthetic UP edge if the button is currently held, then clear
     * the held flag. Safe to call when nothing is held (no-op).
     */
    ...
    /**
     * Intercept hardware key events. Acts on the Samsung Active Key
     * (keyCode 1015) when Samsung ruggedized hardware is detected.
     * Every other keycode returns `false` (not consumed) so normal
     * system key routing is unaffected. On non-Samsung hardware the
     * device gate skips and the service is a no-op regardless of
     * whether the operator has enabled it.
     */
    // Guard-clause early returns (device gate + held-state dedup) read
    ...
    override fun onKeyEvent(event: KeyEvent): Boolean {
        ...
        // hardware key system-wide, so reject anything that isn't the
        // key we handle BEFORE touching the device gate — that reads
        // Build / PackageManager and shouldn't run on every volume or
        // navigation press.
